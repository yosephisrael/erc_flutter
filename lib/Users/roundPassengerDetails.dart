import 'dart:convert';
import 'dart:math';
import 'package:erbs/Users/roundbookSuccess.dart';
...
class RoundPassengerDetail extends StatefulWidget {
  ...
  final String rpdfrom;
  final String rpdto;

  const RoundPassengerDetail({
    Key? key,
    ...
    required this.rpdfrom,
    required this.rpdto,
  }) : super(key: key);

  @override
  _RoundPassengerDetailState createState() => _RoundPassengerDetailState();
}

class _RoundPassengerDetailState extends State<RoundPassengerDetail> {
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const Spacer(),
                        Text(
                          '${(2 * int.parse(widget.pricingDetail)).toString()} ETB', // Double the price
                          style: const TextStyle(
                            fontFamily: 'DMSans',
                            fontWeight: FontWeight.bold,
                            fontSize: 15,
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        ...
                        String time = widget.startingtime;
                        String date = widget.selectejourneydate;
                        String formattedReturnDate = DateFormat('E, MMM d')
                            .format(DateFormat('E, MMM dd')
                                .parse(widget.selectedReturningDate));
                        String newPrice =
                            (int.parse(widget.pricingDetail) * 2).toString();

                        FirebaseFirestore.instance
                            .collection('Round Passengers')
                            .doc(
                                ticketNumber) // Use the unique ticket number as the document ID
                            .set({
                          ...
                          'returnstartpoint': widget.returndestinationingPoint,
                          'returndestinationpoint': widget.returnstartingPoint,
                          'returnstarttime': widget.returnFinishingtime,
                          'returnfinishtime': widget.returnStartingtime,
                        }).then((value) {
                        ...
                        String bodyText =
                            'Ticket number will expire 24 hours after Approval, '
                            'So finish your payment before the deadline please.';
                        QuerySnapshot tokensSnapshot = await FirebaseFirestore
                            .instance
                            .collection("UserTokens")
                            .get();
                        ...
                        for (var doc in tokensSnapshot.docs) {
                          ...
                          sendPushMessage(
                              "Bookingn Confirmation", bodyText, token);
                        }
                      }
                    },
